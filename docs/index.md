...
`robotsix-modules` provides a consistent way to define, validate, and check
module taxonomies across the robotsix ecosystem. It bundles a JSON Schema
(`modules.schema.yaml`) that describes the expected structure of a module
taxonomy file and exposes a public API for loading, validating, and auditing
taxonomy files.
